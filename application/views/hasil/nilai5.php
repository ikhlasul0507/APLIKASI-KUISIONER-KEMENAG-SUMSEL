        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [
                   <?php 
                    $a = $this->db->query('SELECT *FROM tbl_kuisioner WHERE nilai5="a"');
                    $hasil= $a->num_rows();
                    echo $hasil;

                    ?>, 
                    <?php 
                    $b = $this->db->query('SELECT *FROM tbl_kuisioner WHERE nilai5="b"');
                    $hasil= $b->num_rows();
                    echo $hasil;
                    ?>, 
                    <?php 
                    $c = $this->db->query('SELECT *FROM tbl_kuisioner WHERE nilai5="c"');
                    $hasil= $c->num_rows();
                    echo $hasil;
                    ?>,
                    <?php 
                    $d = $this->db->query('SELECT *FROM tbl_kuisioner WHERE nilai5="d"');
                    $hasil= $d->num_rows();
                    echo $hasil;
                    ?>, 
                    ],
